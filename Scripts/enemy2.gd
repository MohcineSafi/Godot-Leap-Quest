...
export var spin = 3
onready var startPosition: float = position.y
onready var target_dist: float = position.y + distance
...
func _physics_process(delta):
	position.y = move_to(position.y, target_dist, speed * delta)
	#have we reached the target?
	if position.y == target_dist:
		#successfully reached the left side
		if target_dist == startPosition:
			#start moving right
			target_dist = position.y + distance
		else:
